...
RUN /opt/radius-venv/bin/pip install --no-cache-dir -r /tmp/ai-requirements.txt \
    && rm /tmp/ai-requirements.txt

COPY . .

RUN groupadd --system radius \
    && useradd --system --gid radius --home-dir /var/www/html --shell /usr/sbin/nologin radius \
...
ENV PATH="/opt/radius-venv/bin:${PATH}"
