...
# How long to wait for Google/SerpApi
SEARCH_TIMEOUT = 10
...
def build_price_search_query(
    ...
    # Used products should search used-market prices.
    # New products can still find retail prices.
...
def normalize_number(value: str) -> float | None:
    ...
    # Avoid obviously impossible values.
...
def extract_prices(text: str) -> list[float]:
    ...
    patterns = [
        ...
        # Generic comma-separated number.
        r"\b(\d{1,3}(?:,\d{3})+(?:\.\d+)?)\b",

        # Generic 5+ digit number.
        r"\b(\d{5,7})\b",
    ]
    ...
    for pattern in patterns:
        ...
        for match in matches:
            ...
            # Practical Bangladesh marketplace range.
...
def live_market_price(
    ...
    #
    # We prioritize relevance rather than blindly taking
    # an unrelated first Google result.
    # --------------------------------------------------------
...
def calculate_price_score(
    ...
    # Listing is dramatically cheaper than market.
    if ratio < 0.35:
        return 95.0
    ...
    # Normal / higher than market.
    if ratio <= 1.25:
        return 8.0

    # Very expensive listings are not necessarily fraud,
    # so keep the risk relatively low.
    if ratio <= 1.60:
        return 12.0
...
def price_risk(
    ...
    # --------------------------------------------------------
    # First try live Google/SerpApi price.
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # Fallback to existing ML dataset.
    # --------------------------------------------------------
    ...
    try:
        ...
        reason = (
            f"Live Google price was unavailable. "
